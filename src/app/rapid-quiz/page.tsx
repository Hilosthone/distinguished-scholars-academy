...
export default function RapidQuizPortal() {
  ...
  const downloadPDF = () => {
    ...
    // --- Watermark Logic ---
    const addWatermark = (pdf: typeof doc) => {
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        // Draw the text at an angle
        // Parameters: text, x, y, options (angle)
        pdf.text('Distinguished Scholars Academy', 45, 190, { angle: 45 });
        
        pdf.restoreGraphicsState(); // Restore original opacity for tables/text
      }
    };
    ...
    doc.text('Examination Result Report', 105, 53, { align: 'center' });
    ...
    autoTable(doc, {
      ...
      theme: 'striped',
      // Trigger watermark when the page is finished
      didDrawPage: (data) => {
        addWatermark(doc);
      }
    });
    ...
    autoTable(doc, {
      ...
      body: getSubjectStats().map(s => [s.name, s.correct, s.total, `${s.percent}%`]),
